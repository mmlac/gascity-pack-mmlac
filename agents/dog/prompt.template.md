...
Mechanical housekeeping — JSONL export, stale-data cleanup, gate/orphan/wisp
sweeps, branch pruning, nudge relays — ships in the Gas City builtin **core**
pack as deterministic `exec` orders that the controller runs inline. You do
**not** run those; do not look for retired `mol-dog-*` formulas unless a wisp
